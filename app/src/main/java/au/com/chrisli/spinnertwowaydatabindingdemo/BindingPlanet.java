...
public class BindingPlanet
{
    ...
    private static class SpinPlanetOnItemSelectedListener implements AdapterView.OnItemSelectedListener
    {
        ...
        public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l)
        {
            if (initialSelectedPlanet_ != null) {
                ...
                Integer positionInAdapter = getPlanetPositionInAdapter((PlanetAdapter) adapterView.getAdapter(), initialSelectedPlanet_);
    ...
    public static void bindPlanetSelected(final AppCompatSpinner spinner, Planet planetSetByViewModel, final InverseBindingListener inverseBindingListener)
    {
        ...
        if (planetSetByViewModel != null && !planetSetByViewModel.equals(spinner.getSelectedItem())) {
            ...
            Integer positionInAdapter = getPlanetPositionInAdapter((PlanetAdapter) spinner.getAdapter(), planetSetByViewModel);
            if (positionInAdapter != null) {
    ...
    private static Integer getPlanetPositionInAdapter(PlanetAdapter adapter, Planet planet)
    {
        if (adapter != null && planet != null) {
            for (int i = 0; i < adapter.getCount(); i++) {
                Planet adapterPlanet = (Planet) adapter.getItem(i);
